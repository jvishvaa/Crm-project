...
const ReAssignLeadToCounsellor = () => {
  ...
  const dropdownData = {
    ...
    pendingType: [
      { label: "None", value: 0 },
      { label: "1-7 Days Pending", value: "1_7_days_pending" },
      { label: "8-15 Days Pending", value: "8_15_days_pending" },
  ...
  const renderFilterView = () => {
    return (
      <Row className="d-flex flex-row align-items-center" gutter={[4, 4]}>
        ...
        {filterData?.pending_type ? (
          <Col>
            <RenderFilterTag
              label="Pending Type"
              value={getArrayValues(
                dropdownData?.pendingType?.filter(
                  (each) => filterData?.pending_type === each?.value
                ),
                "label"
              )?.join(", ")}
            />
